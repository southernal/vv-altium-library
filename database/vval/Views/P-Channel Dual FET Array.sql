...
AS
     SELECT TOP (100) PERCENT PartId,
                              ...
                              RdsOnMax AS [Rds On (Max) at Id, Vgs],
                              VgsthMax AS [Vgs(th) (Max) at Id],
                              GateCharge AS [Gate Charge (Qg)],
                              InputCapacitance AS [Input Capacitance (Ciss) at Vds],
                              PowerMax AS [Power - Max],
                              DrainToSourceVoltage AS [Drain to Source Voltage (Vdss)],
                              Features,
                              CurrentRating AS [Current Continuous Drain]
     FROM dbo.Components
     WHERE(ComponentType = 'FET')
          AND (ComponentSubType = 'PCH-DUAL');
